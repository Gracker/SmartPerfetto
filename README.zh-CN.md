...
| 后端 | Node.js 20+、TypeScript strict mode、Express |
...
只想把 SmartPerfetto 跑起来时，推荐使用这个方式。你只需要 Docker Desktop/Engine 和一个大模型 API Key；不需要安装 Node.js，不需要 C++ 工具链，也不需要初始化 `perfetto/` submodule。Docker Hub 镜像每天从 `main` 自动发布，镜像内已经包含后端、预构建 Perfetto UI 和固定版本的 `trace_processor_shell`。
...
如果你希望直接从源码 checkout 启动，使用这个方式。前置条件：**Node.js 20+**、`curl`、`lsof`、`pkill`，以及一个大模型 API Key。Windows 源码开发请使用 [WSL2](https://learn.microsoft.com/zh-cn/windows/wsl/install)，不要使用原生 Windows shell。
