...
前置条件：**Node.js 18+**、`curl`、`lsof`、`pkill`，以及一个大模型 API Key。

> **平台支持**：完整支持 macOS 和 Linux（Ubuntu/Debian）。Windows 用户请使用 [WSL2](https://learn.microsoft.com/zh-cn/windows/wsl/install) 或 Docker。
...
仓库内置了预编译的 Perfetto UI（`frontend/` 目录）——**不需要初始化 submodule，不需要 C++ 工具链，不需要 30 分钟编译**。加载 `.pftrace` 文件，打开 AI Assistant 面板即可开始分析。
