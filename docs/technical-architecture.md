...
│  └─ 自然语言交互              └─ 数据摘要与压缩            │
│                                                         │
│  连接层: MCP (Model Context Protocol) — 18 个工具        │
│  质量层: 4 层验证 + 反思纠错                              │
└─────────────────────────────────────────────────────────┘
```
...
| **MCP Server** | `claudeMcpServer.ts` | 18 个工具的注册与执行。Claude 通过 MCP 协议调用 |
...
MCP (Model Context Protocol) 是 Anthropic 推出的标准化协议，让 LLM 可以调用外部工具。SmartPerfetto 注册了 **18 个 MCP 工具**，这是 Claude 与 trace 数据之间的唯一桥梁。
...
| 工具 | 用途 |
|------|------|
| `recall_patterns` | 跨会话分析模式记忆 (正面/负面经验) |
| *(学习系统)* | 自动从验证结果中提取误诊模式 (90 天 TTL) |
...
| **Pipeline** | 30 | `skills/pipelines/` | 渲染管线检测 + 教学 (30 种管线类型) |
| **总计** | **140** | | |
...
SmartPerfetto 支持 **30 种** Android 渲染管线的自动识别和教学：
...
| `analysis_completed` | 分析完成 | 携带 reportUrl |
| `error` | 异常 | 任何错误 |
...
│                   │ Verifier             │ 质量保证           │
├─────────────────────────────────────────────────────────────┤
│ 工具桥接层         │ MCP Server (18工具)  │ LLM ↔ 数据桥梁    │
│                   │ Artifact Store       │ Token 节省         │
│                   │ SQL Summarizer       │ 数据压缩           │
├─────────────────────────────────────────────────────────────┤
│ 领域知识层         │ 140 Skills (YAML)    │ 分析逻辑复用       │
