...
| 类型 | 数量 | 位置 | 说明 |
|------|------|------|------|
| **Atomic** | 80 | `skills/atomic/` | 单步 SQL 查询 (有些含多个 steps) |
| **Composite** | 28 | `skills/composite/` | 多步编排 (iterator/parallel/conditional) |
| **Deep** | 2 | `skills/deep/` | 深度分析 (CPU profiling, callstack) |
| **Pipeline** | 30 | `skills/pipelines/` | 渲染管线检测 + 教学内容 |
| **总计** | **140** | | |
...
```yaml
display:
  layer: overview              # overview | list | session | deep
  level: summary               # none | debug | detail | summary | key
  title: "帧率概览"             # 显示标题
  format: table                # table | chart | text | timeline | summary
  columns:                     # 列定义
    - name: ts
      label: "时间戳"
      type: timestamp           # timestamp | duration | number | string | percentage | bytes
      clickAction: navigate_timeline  # navigate_timeline | navigate_range | copy
    - name: dur_ms
      label: "耗时"
      type: duration
      unit: ms                  # ns | us | ms | s
...
前端根据 `display.columns` 的类型和 `clickAction` **自动渲染**表格、跳转链接、格式化数值——不需要为每个 Skill 写专门的 UI 代码。
...
Pipeline Skills 是一种特殊的 Skill 类型，用于 **30 种 Android 渲染管线**的自动识别和教学。
