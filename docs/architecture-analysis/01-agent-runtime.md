...
干预选项通过 SSE `intervention_required` 事件推送到前端，用户可通过 `POST /api/agent/v1/:sessionId/intervene` 响应。
...
| DeepSeek | `deepseek-reasoner` | Yes | reasoning, coding |
| DeepSeek | `deepseek-coder` | Yes | coding, cost |
| GLM (智谱) | `glm-5` | Yes | reasoning, coding, cost |
...
- `base_url` supports `https://api.deepseek.com` and `https://api.deepseek.com/v1`.
- `/v1` is API compatibility path only, and is unrelated to model version.
- `deepseek-chat` and `deepseek-reasoner` both map to DeepSeek-V3.2 (128K context):
...
**故障转移链（默认）：** `glm-5 → deepseek-reasoner → deepseek-chat`
...
- 前端可通过 `POST /api/agent/v1/:sessionId/interaction` 上报点击事件
...
| `POST` | `/api/agent/v1/:sessionId/respond` | 响应断路器（continue/abort） |
| `DELETE` | `/api/agent/v1/:sessionId` | 清理会话 |

### 8.2 Intervention & Focus (v2.0)

| Method | Path | 描述 |
|--------|------|------|
| `POST` | `/api/agent/v1/:sessionId/intervene` | 响应干预请求 |
| `POST` | `/api/agent/v1/:sessionId/interaction` | 上报用户交互（点击时间戳等） |
| `GET` | `/api/agent/v1/:sessionId/focus` | 获取当前焦点列表 |
...
| `GET` | `/api/agent/v1/scene-reconstruct/:id/stream` | 场景重建 SSE 流 |
| `GET` | `/api/agent/v1/scene-reconstruct/:id/tracks` | 获取轨道事件 |
| `GET` | `/api/agent/v1/scene-reconstruct/:id/status` | 场景重建状态 |
| `DELETE` | `/api/agent/v1/scene-reconstruct/:id` | 清理场景重建会话 |
...
| Method | Path | 描述 |
|--------|------|------|
| `POST` | `/api/agent/v1/teaching/pipeline` | 渲染管线检测与教学 |

### 8.5 Session & Log 管理

| Method | Path | 描述 |
|--------|------|------|
| `GET` | `/api/agent/v1/sessions` | 列出会话（含可恢复会话） |
| `POST` | `/api/agent/v1/resume` | 恢复会话 |
| `GET` | `/api/agent/v1/:sessionId/report` | 生成 JSON 报告 |
| `GET` | `/api/agent/v1/logs` | 列出所有日志 |
| `GET` | `/api/agent/v1/logs/:sessionId` | 查看会话日志 |
| `GET` | `/api/agent/v1/logs/:sessionId/errors` | 仅错误/警告 |
| `POST` | `/api/agent/v1/logs/cleanup` | 清理过期日志 |
