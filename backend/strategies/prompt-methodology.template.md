...
- `cpu_frequency_counters` — CPU 频率变化区间（ts, dur, freq, cpu）— 用法见下方 SQL 陷阱
...
- **`counter` 表没有 `cpu` 列**：CPU 信息在 `counter_track.name` 中（如 `cpufreq`）。查 CPU 频率请用 stdlib `cpu_frequency_counters`（`INCLUDE PERFETTO MODULE linux.cpu.frequency`），它提供 `cpu`, `ts`, `dur`, `freq` 列
...
- **`cpu_frequency_counters` 必须先 INCLUDE 模块**：写法是 `INCLUDE PERFETTO MODULE linux.cpu.frequency;` 然后才能 `SELECT ... FROM cpu_frequency_counters`。这个表**不在**自动预加载列表中。直接 SELECT 会报 "no such table: cpu_frequency_counters"
